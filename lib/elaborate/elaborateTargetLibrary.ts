...
export class ElaborateTargetLibrary {
  ...
  public elaborate() {
    ...
    // no library at all defined
    if (fileTargetLibrary === undefined && this.vhdlLinter.file.targetLibrary === undefined) {
      this.vhdlLinter.addMessage({
        message: `This file has no target library assigned.`,
    ...
    if (this.vhdlLinter.file.targetLibrary !== undefined && fileTargetLibrary !== undefined && this.vhdlLinter.file.targetLibrary !== fileTargetLibrary?.library) {
      this.vhdlLinter.addMessage({
        message: `The library assigned in the magic comment (${this.vhdlLinter.file.targetLibrary}) does not match ${fileTargetLibrary.library} from ${fileURLToPath(fileTargetLibrary.definitionFile)}:${fileTargetLibrary.definitionLine}.`,
        range: new OIRange(this.vhdlLinter.file, 0, this.vhdlLinter.file.lines[0]?.length ?? 1),
