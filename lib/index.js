...
class VhdlLinter {
  ...
  parser() {
    ...
    for (const [lineNumber, line] of this.lines.entries()) {
      ...
      if (mode == 'root') {
        ...
        const ifMatch = line.match(/^\s*(if|elsif)[^a-z0-9_](.*)then/i);
        if (ifMatch !== null) {
          const condition = ifMatch[2].replace(/^\s*\(([^)]*)\)\s/i, (_, con) => con);
          for (const read of this.parseRead(condition, lineNumber)) {
            this.reads.push({
              name: read,
  ...
  checkUnused() {
    for (const wire of this.wires) {
      if (wire.type === 'generic') {
        continue;
      }
      const wireName = wire.name.toLowerCase();
      if (typeof this.reads.concat(this.writes).find(readWrite => readWrite.nameLowerCase === wireName) === 'undefined') {
        this.messages.push({
          severity: 'warning',
          location: {
            file: this.editorPath,
