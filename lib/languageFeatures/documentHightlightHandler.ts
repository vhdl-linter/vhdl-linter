...
import { OWriteReadBase, OWrite } from '../parser/objects';
...
export async function documentHighlightHandler(params: TextDocumentPositionParams): Promise<DocumentHighlight[] | null> {
    ...
    if (!candidate || !(candidate instanceof OWriteReadBase)) {
        return null;
    }
    return (linter.tree.objectList.filter(object => object instanceof OWriteReadBase && object.text.toLowerCase() === candidate.text.toLowerCase()) as OWriteReadBase[])
        .map(object => ({
            range: object.range,
