...
export class ArchitectureParser extends ParserBase {
  ...
  parse(skipStart: boolean, structureName: 'generate', variable: { variable: string, variableRange: ORead[], startPosI: number }): OForGenerate;
  parse(skipStart = false, structureName: 'architecture' | 'generate' | 'block' = 'architecture', forShit?: { variable: string, variableRange: ORead[], startPosI: number }): OArchitecture | OForGenerate | OIfGenerateClause {
    this.debug(`parse`);
    if (structureName === 'architecture') {
      ...
      const { variable, variableRange, startPosI } = forShit as { variable: string, variableRange: ORead[], startPosI: number };
      this.architecture = new OForGenerate(this.parent as OArchitecture, this.pos.i, this.getEndOfLineI(), variableRange);
