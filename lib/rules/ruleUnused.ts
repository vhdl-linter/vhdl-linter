...
import { IHasLexerToken, IHasPorts, IHasNameToken, implementsIHasDeclarations, implementsIHasGenerics, implementsIHasLexerToken, implementsIHasPorts } from "../parser/interfaces";
...
export class RuleUnused extends RuleBase implements IRule {
  ...
  private checkUnusedPorts(obj: ObjectBase & IHasPorts) {
    ...
    for (const port of obj.ports) {
      const type = port.typeNames[0]?.definitions?.[0];
        ...
        for (const declaration of obj.declarations) {
          ...
          if (declaration instanceof OVariable) {
            ...
            if (references.filter(token => token.write).length === 0) {
              ...
              const type = declaration.typeNames[0]?.definitions?.[0];
